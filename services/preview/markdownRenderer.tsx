...
const MarkdownViewer = forwardRef<HTMLDivElement, MarkdownViewerProps>(({ content, settings, onScroll }, ref) => {
  ...
  const components = useMemo<Components>(() => {
    ...
    const isPlantUmlLanguage = (
      value: string
    ): value is (typeof PLANTUML_LANGS)[number] =>
      (PLANTUML_LANGS as ReadonlyArray<string>).includes(value);

    const preComponent: Components['pre'] = ({ children, className, node: _node, ...props }) => {
      ...
      const codeClassName =
        typeof codeProps.className === 'string' ? (codeProps.className as string) : undefined;
      ...
      const baseClassName = mergeClassNames('df-code-block', className, codeClassName);
      ...
      if (normalizedLanguage && highlighter) {
        try {
          ...
          const { onScroll, ...restPreProps } = props as ComponentPropsWithoutRef<'pre'>;
          const forwardedDivProps =
            restPreProps as unknown as React.HTMLAttributes<HTMLDivElement>;

          return (
            <div
              {...forwardedDivProps}
              className={mergeClassNames(baseClassName, 'df-code-block-shiki')}
          ...
          console.warn(
            '[MarkdownRenderer] Failed to highlight code block for language ' + language + ':',
            error
          );
        }
      }

      const { children: _codeChildren, inline: _inline, node: _codeNode, ...restCodeProps } =
        codeProps;
      ...
      return (
        ...
        >
          <code
            className={mergeClassNames('df-code-block__code', codeClassName)}
    ...
    const tableComponent: Components['table'] = ({ children, className, ...props }) => (
    ...
    const theadComponent: Components['thead'] = ({ children, className, ...props }) => (
      <thead className={mergeClassNames('df-table-head', className)} {...props}>
        {children}
      </thead>
    );

    const tbodyComponent: Components['tbody'] = ({ children, className, ...props }) => (
      <tbody className={mergeClassNames('df-table-body', className)} {...props}>
        {children}
      </tbody>
    );

    const trComponent: Components['tr'] = ({ children, className, ...props }) => (
      <tr className={mergeClassNames('df-table-row', className)} {...props}>
        {children}
      </tr>
    );

    const thComponent: Components['th'] = ({ children, className, ...props }) => (
      <th className={mergeClassNames('df-table-header', className)} {...props}>
        {children}
      </th>
    );

    const tdComponent: Components['td'] = ({ children, className, ...props }) => (
      <td className={mergeClassNames('df-table-cell', className)} {...props}>
        {children}
      </td>
    );

    const blockquoteComponent: Components['blockquote'] = ({ children, className, ...props }) => (
      <blockquote className={mergeClassNames('df-blockquote', className)} {...props}>
        {children}
      </blockquote>
    );

    const linkComponent: Components['a'] = ({ children, className, ...props }) => (
    ...
    const imgComponent: Components['img'] = (props) => (
      <img className="df-image" loading="lazy" {...props} />
    );

    const unorderedListComponent: Components['ul'] = ({ children, className, ...props }) => (
      <ul className={mergeClassNames('df-list', 'df-list-disc', className)} {...props}>
        {children}
      </ul>
    );

    const orderedListComponent: Components['ol'] = ({ children, className, ...props }) => (
      <ol className={mergeClassNames('df-list', 'df-list-decimal', className)} {...props}>
        {children}
      </ol>
    );

    const listItemComponent: Components['li'] = ({ children, className, ...props }) => (
      <li className={mergeClassNames('df-list-item', className)} {...props}>
        {children}
      </li>
    );

    const hrComponent: Components['hr'] = ({ className, ...props }) => (
      <hr className={mergeClassNames('df-divider', className)} {...props} />
    );
