...
function buildAssetLookup(assets, metadataDir) {
  const assetByName = new Map();

  for (const asset of assets) {
    if (asset.directory !== metadataDir) {
      continue;
    }
  ...
  for (const asset of assets) {
    if (!assetByName.has(asset.fileName)) {
      assetByName.set(asset.fileName, asset);
    }
    const originalName = asset.originalFileName;
    if (originalName && !assetByName.has(originalName)) {
      assetByName.set(originalName, asset);
    }
  }
...
async function updateMetadataFiles(metadataFiles, releaseAssets) {
  ...
  const ensureEntryMatchesAsset = (entry, assetByName) => {
    if (!entry) {
      return false;
    }
  ...
  for (const metadataPath of metadataFiles) {
    ...
    const assetByName = buildAssetLookup(releaseAssets, metadataDir);
    ...
    if (Array.isArray(parsed.files)) {
      for (const entry of parsed.files) {
        if (ensureEntryMatchesAsset(entry, assetByName)) {
          changed = true;
        }
      }
    }
