      ...
      runRemoteCheck({
        ...
        http: {
          fetchJson: async () => ({
            assets: [
              {
                name: 'DocForge-Setup-0.7.0.exe',
                browser_download_url: 'https://example.invalid/DocForge-Setup-0.7.0.exe',
              },
            ],
          }),
        },
...
test('remote auto-update check falls back to latest release when requested tag is missing', async () => {
  ...
  const http = {
    fetchJson: async (url) => {
      if (url.endsWith('/releases/tags/v0.7.0')) {
...
test('auto-update analysis reports unreachable assets when GitHub returns 404', async () => {
  const metadataSource = YAML.stringify({
    ...
    files: [
      {
        url: 'DocForge-Setup-0.7.0.exe',
        sha512: 'placeholder',
        size: 100,
      },
    ],
    path: 'DocForge-Setup-0.7.0.exe',
    sha512: 'placeholder',
  });

  const assets = new Map([
    [
      'DocForge-Setup-0.7.0.exe',
      {
        name: 'DocForge-Setup-0.7.0.exe',
        browser_download_url: 'https://example.invalid/DocForge-Setup-0.7.0.exe',
      },
  ...
  assert(result.unreachable.some((entry) => entry.includes('DocForge-Setup-0.7.0.exe') && entry.includes('404 (Not Found)')));
});
