...
async function writeFixtureInstaller(directory, version, options = {}) {
  const {
    artifactName = 'docforge-windows-x64',
    installerName = `DocForge Setup ${version}.exe`,
    metadataName = 'latest.yml',
  } = options;

  const releaseDir = path.join(directory, 'release-artifacts', artifactName, 'release');
  await fs.mkdir(releaseDir, { recursive: true });
  const originalName = installerName;
  ...
  const latestPath = path.join(releaseDir, metadataName);
...
test('metadata rewriting keeps per-architecture hashes distinct when installers share names', async (t) => {
  const workspace = await createTemporaryWorkspace(t);

  const x64 = await writeFixtureInstaller(workspace, '0.0.3', {
    artifactName: 'docforge-windows-x64',
  });

  const ia32 = await writeFixtureInstaller(workspace, '0.0.3', {
    artifactName: 'docforge-windows-ia32',
  });

  const changelogPath = path.join(workspace, 'CHANGELOG.md');
  await fs.writeFile(
    changelogPath,
    ['## v0.0.3', '', '- Regression coverage for duplicate installer names.'].join('\n'),
    'utf8',
  );
  ...
  const renamedX64Installer = path.join(x64.releaseDir, 'DocForge-Setup-0.0.3.exe');
  const renamedIa32Installer = path.join(ia32.releaseDir, 'DocForge-Setup-0.0.3.exe');

  const [x64Buffer, ia32Buffer] = await Promise.all([
    fs.readFile(renamedX64Installer),
    fs.readFile(renamedIa32Installer),
  ]);

  const x64Sha = computeSha512Base64(x64Buffer);
  const ia32Sha = computeSha512Base64(ia32Buffer);

  assert.notEqual(x64Sha, ia32Sha, 'Fixtures must differ to confirm hashes stay distinct');

  const x64Metadata = YAML.parse(await fs.readFile(x64.latestPath, 'utf8'));
  const ia32Metadata = YAML.parse(await fs.readFile(ia32.latestPath, 'utf8'));

  assert.equal(x64Metadata.path, 'DocForge-Setup-0.0.3.exe');
  assert.equal(x64Metadata.sha512, x64Sha);
  assert.equal(x64Metadata.files[0].url, 'DocForge-Setup-0.0.3.exe');
  assert.equal(x64Metadata.files[0].sha512, x64Sha);
  assert.equal(x64Metadata.files[0].size, x64Buffer.length);

  assert.equal(ia32Metadata.path, 'DocForge-Setup-0.0.3.exe');
  assert.equal(ia32Metadata.sha512, ia32Sha);
  assert.equal(ia32Metadata.files[0].url, 'DocForge-Setup-0.0.3.exe');
  assert.equal(ia32Metadata.files[0].sha512, ia32Sha);
  assert.equal(ia32Metadata.files[0].size, ia32Buffer.length);
