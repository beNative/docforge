...
-- PRAGMA user_version is set to 5 by the database service for this schema.
...
-- =================================================================
--  GENERIC SCRIPT EXECUTION (Shell & PowerShell)
-- =================================================================
CREATE TABLE script_execution_runs (
    run_id              TEXT PRIMARY KEY,
    node_id             TEXT NOT NULL REFERENCES nodes(node_id) ON DELETE CASCADE,
    language            TEXT NOT NULL CHECK(language IN ('shell', 'powershell')),
    status              TEXT NOT NULL,
    started_at          TEXT NOT NULL,
    finished_at         TEXT,
    exit_code           INTEGER,
...
CREATE INDEX idx_script_runs_node_language ON script_execution_runs(node_id, language);
